...
userName=`grep tokenid /var/tmp/tokenData | cut -d'!' -f1 | cut -d' ' -f4`
tokenName=`grep tokenid /var/tmp/tokenData | cut -d'!' -f2 | cut -d' ' -f1`
token=`grep value /var/tmp/tokenData | grep -v key | cut -d' ' -f11`
